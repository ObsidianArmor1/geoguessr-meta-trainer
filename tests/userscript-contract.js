...
assert.match(source, /src\/cradio-client\.js\?v=2\.2\.0-beta\.71/,
...
assert.match(source, /src\/lodestar-pack-v2\.js\?v=2\.2\.0-beta\.71/,
  "Tampermonkey receives the cache-preserving Pack V2 client in this release");
assert.match(source, /probePrivateLayer\(\);/,
  "private-layer availability is determined while the player is still in the lobby");
assert.match(source, /PRIVATE_LAYER_REQUEST_TIMEOUT_MS = 1_500/,
  "a wedged loopback service cannot impose a multi-second fallback penalty");
